...
- [ ] All puzzle logic can go into `/controllers/sudoku-solver.js`
  - [X] ~~*The `validate` function should take a given puzzle string and check it to see if it has 81 valid characters for the input.*~~ [2022-01-10]
  - [X] ~~*The `check` functions should be validating against the *current* state of the board.*~~ [2022-01-10]
  - [X] ~~*The `solve` function should handle solving any given valid puzzle string, not just the test inputs and solutions. You are expected to write out the logic to solve this.*~~ [2022-01-10]
